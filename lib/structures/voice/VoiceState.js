...
class VoiceState extends base_1.BaseStruct {
    ...
    init(voiceState) {
        if (voiceState.self_mute)
            this.muted = new MuteFlags_1.MuteFlags(MuteFlags_1.MUTE_STATE.SELF);
        if (voiceState.mute)
            this.muted = new MuteFlags_1.MuteFlags(MuteFlags_1.MUTE_STATE.FORCE);
        if (voiceState.self_deaf)
            new MuteFlags_1.MuteFlags(MuteFlags_1.MUTE_STATE.SELF);
        if (voiceState.deaf)
            this.deafen = new MuteFlags_1.MuteFlags(MuteFlags_1.MUTE_STATE.FORCE);
