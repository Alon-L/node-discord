...
class Guild extends GuildPreview_1.GuildPreview {
    ...
    init(guild) {
        ...
        const nonVoicedMembers = this.members.cache
            .filter(x => !x.voice)
            .map(x => Object.assign(x, guild.members.find((y) => y.id === x.id))).toArray;
        for (const nonVoiceMember of nonVoicedMembers) {
            this.voiceStates.set(nonVoiceMember.id, new VoiceState_1.VoiceState(this.bot, nonVoiceMember, {
